...
public class UICollectionView : MonoBehaviour {
    ...
    [SerializeField]
    public CollectionViewLayout m_layout;
    ...
    private void KeyboardInput()
    {
        ...
        {
            if(Keyboard.current.rightArrowKey.wasPressedThisFrame)// Input.GetKeyDown(KeyCode.RightArrow))
            {
                StepRight();
        ...
        if(m_layout!=null && Data.Count > 0)
        {
            if(m_dragState == DragState.None)
            {
                ...
                if(Mouse.current.leftButton.wasPressedThisFrame)// Input.GetMouseButtonDown(0))
                {
                    ...
                    RaycastHit hit;
                    Ray r = cam.ScreenPointToRay(Mouse.current.position.ReadValue());//Input.mousePosition);
                    if(Physics.Raycast(r, out hit, float.MaxValue, m_inputDragLayer.value))
                    {
                        //Click start
                        m_dragState = DragState.Starting;
                        m_totalDrag = Vector2.zero;
                        m_lastScreenPos = Mouse.current.position.ReadValue();
                    }
                }
            }
            else
            {
                ...
                {
                    //Drag
                    Vector2 newPoint = Mouse.current.position.ReadValue();
    ...
    private void TouchDrag()
    {
        ...
        TouchControl t = Touchscreen.current.touches[0];//Input.touches[0];
        ...
        if (m_layout!=null && Data.Count > 0)
        {
            if(m_dragState == DragState.None)
            {
                ...
                if(t.phase.ReadValue() == TouchPhase.Began)
                {
                    //See if we click on any cells, and snap to that
                    Camera cam = (m_inputCamera!=null) ? m_inputCamera : Camera.main;
                    
                    RaycastHit hit;
                    Vector3 point = t.position.ReadValue();//new Vector3(t.position.x, t.position.y, 0);
                    ...
                    if(Physics.Raycast(r, out hit, float.MaxValue, m_inputDragLayer.value))
                    {
                        ...
                        m_dragState = DragState.Starting;
                        m_totalDrag = Vector2.zero;
                        m_lastScreenPos = t.position.ReadValue(); 
                    }
                }
            }
            else
            {
                if(t.phase.ReadValue() == TouchPhase.Moved)
                {
                    //Drag
                    Vector2 newPoint = t.position.ReadValue();
                    Vector2 delta = m_lastScreenPos - newPoint;
                ...
                else if(t.phase.ReadValue() != TouchPhase.Stationary)
                {
                    StopDrag();
                }
            }
